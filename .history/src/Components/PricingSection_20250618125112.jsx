      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 md:gap-8 items-stretch">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`relative flex flex-col p-4 sm:p-4 md:p-6 rounded-2xl md:rounded-3xl border transition-transform duration-200 bg-white/90 shadow-xl hover:scale-[1.03] w-full max-w-xs sm:max-w-md mx-auto h-full ${
                plan.recommended
                  ? "border-4 border-indigo-500 bg-indigo-50 z-20 md:scale-107 shadow-2xl"
                  : "border-gray-200"
              }`}
            >
              {plan.recommended && (
