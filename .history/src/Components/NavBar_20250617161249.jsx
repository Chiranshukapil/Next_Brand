      ...
      <nav className="navbar shadow-md sticky top-0 z-50 backdrop-blur-md bg-opacity-90 transition-all duration-300">
        ...
        <div className="navbar-end">
          ...
          <div className="md:hidden">
            ...
            {/* Mobile Menu with max-height transition */}
            <div
              className={`overflow-hidden transition-all duration-300 ease-in-out right-0 left-auto ${
                menuOpen ? "max-h-96" : "max-h-0"
              }`}
            >
              <ul
                tabIndex={0}
                className="menu menu-sm mt-3 p-2 shadow-lg bg-white rounded-box w-56 "
              >
                {navLinks.map((link) => (
                  <li key={link.name}>
                    {link.dropdown ? (
